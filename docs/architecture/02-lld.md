...
│   │   │       ├── payments.py
│   │   │       └── users.py
│   │   ├── auth/                     # fastapi-users auth module
│   │   │   ├── __init__.py
│   │   │   ├── config.py             # FastAPIUsers setup
...
We use `fastapi-users` for authentication - a batteries-included library that provides:

| Feature | Implementation |
|---------|---------------|
| **Google OAuth** | One-click login via `httpx-oauth` |
| **Magic Link** | Passwordless email login via Resend |
| **JWT Tokens** | Access + refresh token strategy |

**Auth Endpoints (auto-generated by fastapi-users):**

| Method | Endpoint | Description |
|--------|----------|-------------|
| `POST` | `/auth/jwt/login` | Email + password login |
| `POST` | `/auth/jwt/logout` | Logout |
| `POST` | `/auth/forgot-password` | Send magic link |
| `POST` | `/auth/reset-password` | Verify magic link |
...
async def create_quest(
    data: QuestCreate,
    user: User = Depends(current_active_user),  # Requires login
):
